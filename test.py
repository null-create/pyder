...
def send_request(url, session_method, headers, params=None):
    if params:
        response = session_method(
            ...
            headers=headers,
            stream=True,
            params={"variables": json.dumps(params), "features": json.dumps(features)},
        )
    else:
    ...
    ), f"Failed request to {url}.  {response.status_code}.  Please submit an issue including this information. {response.text}"
...
def get_tweets(query_id, session, headers, variables, expected_total):
    ...
    all_tweets = [
        ...
        if "full_text" in tweet and tweet.get("user_id_str", "") == variables["userId"]
    ]
    return all_tweets
...
def get_id_and_tweet_count(session, headers, query_id, username):
    ...
    assert (
        len(ids) == 1
    ), f"Failed to find user id for {username}.  Please open this as an issue including this message."

    counts = COUNT_PATTERN.findall(resp)
    assert (
        len(counts) == 1
    ), f"Failed to find tweet count for {username}.  Please open this as an issue including this message."
...
def user_tweets(username):
    ...
    assert (
        bearer_token
    ), f"Did not find bearer token.  Are you sure you used the right username? {username}"
    assert (
        query_id
    ), f"Did not find query id.  Are you sure you used the right twitter username? {username}"
    assert (
        user_query_id
    ), f"Did not find user query id.  Are you sure you used the right twitter username? {username}"
    ...
    assert (
        guest_token
    ), f"Did not find guest token.  Probably means the script is broken.  Please submit an issue.  Include this message in your issue: {username}"
...
if __name__ == "__main__":
    ...
    # group.add_argument(
    #     "--username", help="The username of the user to get tweets for.", required=False
    # )
    # group.add_argument(
    #     "--usersFile",
    ...
    # parser.add_argument(
    #     "--output",
    #     help="The output file to write to.  If not specified, prints to stdout.",
    # )
    ...
    writer = csv.DictWriter(open(output, "w"), fieldnames=headers)
    writer.writeheader()
